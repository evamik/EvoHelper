...
export const CharacterProvider: FC<PropsWithChildren> = ({ children }) => {
  ...
  const [accountsDict, setAccountsDict] = useState<{
    [account: string]: IClassLoad[];
  }>({});
  const [classesList, setClassesList] = useState<
    Array<{ name: string; tier: number }>
  >([]);
  const loadClasses = () => {
    window.electron.ipcRenderer.sendMessage(
      'loadData',
      // probably should get rid of contant path parts in 'frontend'
      [wc3path, 'CustomMapData', "Twilight's Eve Evo"],
    );
  };
  const getCharacter = (account?: string, id?: string) => {
    ...
    const formattedAccount =
      account.indexOf('$') !== -1 ? account.replace('$', '#') : account;

    return accountsDict[formattedAccount]?.find(
      (character) => character.hero === id,
    );
  };
  const onLoadClick = (character: IClassLoad, legacy?: boolean) => {
    if (character && character.code) {
      ...
      window.electron.ipcRenderer.sendMessage(
        'load',
        ['-rp', '-lc', ...loadCodeChunks, '-le', ...extraLines.split('\n')],
        legacy,
      );
    }
  };
  ...
  useEffect(() => {
    const handleLoadData = (arg: any) => {
      setAccountsDict((prev) => {
        if (JSON.stringify(prev) !== JSON.stringify(arg)) {
          return arg;
        }
        return prev;
      });
    ...
    window.electron.ipcRenderer.on('loadData', handleLoadData);
